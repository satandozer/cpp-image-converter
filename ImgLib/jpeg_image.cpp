...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        //std::cerr << "writing line: " << cinfo.next_scanline << std::endl;
        auto line = image.GetLine(cinfo.next_scanline);
        for (int i = 0; i < image.GetWidth(); ++i){
            buffer[i* 3 + 0] = static_cast<JSAMPLE>(line[i].r);
            buffer[i* 3 + 1] = static_cast<JSAMPLE>(line[i].g);
            buffer[i* 3 + 2] = static_cast<JSAMPLE>(line[i].b);
        }
